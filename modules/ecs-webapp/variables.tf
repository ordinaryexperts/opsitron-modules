...
variable "stopped_message_html" {
  description = "HTML body returned by the ALB when `enabled = false`, intercepting all traffic before it reaches the (empty) target group. Capped at 1024 bytes by ALB. Override per-app for branding; leave null/unset to use the canned 'environment paused' page (see local.stopped_message_html in alb.tf)."
  type        = string
  default     = null
  nullable    = true

  validation {
    condition     = var.stopped_message_html == null ? true : length(var.stopped_message_html) <= 1024
    error_message = "stopped_message_html must be 1024 bytes or fewer (ALB fixed-response limit), or null to use the module default."
  }
}
